...
-- Scenario 1 - Sebquery in 'SELECT'
-- Show each job's salary next to overall market median;
SELECT 
    job_title_short,
    salary_year_avg,
    (
...
-- CTE EXAMPLE
-- Compare how much more (or less) remote roles pay compare to omsite roles for each job tittle
...
-- CTE JOINS FOR SOURCE TABLE TO TARGET TABLE
SELECT *
...
-- FINAL EXAMPLE
-- Identify job postings that have no associated skills before loading them into a datamart
SELECT * 
FROM skills_job_dim
ORDER BY job_id
LIMIT 40;
